...
| `update task list` | 更新 `TaskList`或`Web-TaskList`完成进度 | 根据当前代码完成情况同步任务状态 |
...
1. 主动询问用户是否需要进行如下保存操作：
  ...
  - 是否更新`TaskList`，`Web-TaskList`中完成进度
...
- **任务分解与规划**:
    1.  在接收到复杂任务时（例如“实现一个新功能”或“重构某个模块”），优先使用**skill:`superpowers`**，首先要向用户确认已经理解了核心目标。
    2.  然后，查阅 `docs` 目录下的相关文档（如需求、设计文档）以获取上下文。
    3.  基于目标和上下文，生成一个分步骤的执行计划，并以 `TaskList.md`或 `Web-TaskList.md` 的格式呈现。
...
- 对关键状态、进展和决策进行结构化总结
- 不擅自修改 `daily-sessions`、`daily-report` 和 `TaskList`， `Web-TaskList`，除非用户确认或当前流程明确要求
- 保持输出简洁、清晰、可追踪
- 对不确定的信息进行标注说明
- 设计方案时，在给出选项的同时必须给出选项的优缺点，并给出建议和原因
- 当前任务完成后，如有明确进度变化，需要同步 `TaskList` 和`Web-TaskList`，并给出下一步建议
...
- `daily-sessions`（短期记忆）：
  ...
  - `下一步` 必须优先引用 `docs/TaskList.md`, `docs/Web-TaskList.md` 中的任务编号，例如 `NTL-S0-007`
...
- `daily-report`（长期记录）：
  ...
  - `下一步` 必须按优先级列出 3 到 6 条，并尽量引用 `docs/TaskList.md`, `docs/Web-TaskList.md` 中的任务编号
