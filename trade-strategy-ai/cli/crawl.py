...
def run_crawl_command(*, config_path: Path, max_articles: int | None = None, force: bool = False) -> list[str]:
...
def crawl(
    ...
    force: bool = typer.Option(False, help="是否强制重跑并清理旧状态"),
    log_level: str = typer.Option("INFO", help="日志级别"),
) -> None:
    configure_logging(log_level)
    for line in run_crawl_command(config_path=config, max_articles=max_articles, force=force):
        typer.echo(line)
