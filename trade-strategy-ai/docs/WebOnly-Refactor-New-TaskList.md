...
1. Process 保持和 extract-articles 一致
建议：强烈推荐保持一致。

目前 process 和 extract-articles 在数据流和产物上有差异，容易导致规则池、元数据等不一致，影响后续回测和策略生成。
建议将 process 步骤的行为、产物（如 rule_pool、article_metadata 等）与 extract-articles 完全对齐，确保所有链路（pipeline、调度、手动补跑）产物一致，便于维护和追溯。
这样可以避免“某些规则只在 extract-articles 跑出来，process 跑不出来”的隐患。
...
总结与建议
Process 和 extract-articles 保持一致，有助于数据一致性和后续流程稳定。
盘后自动补全增量数据，是保证盘后优化和评估链路可靠的基础。
这两项都是提升系统健壮性和自动化程度的关键改进，建议尽快纳入开发计划。
