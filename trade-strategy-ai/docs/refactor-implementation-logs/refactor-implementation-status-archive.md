...
- Task ID: `POST-DELIVERY-HOTFIX Research center page follow-up polish`
- 状态: `已完成`
- 修改范围: 研究中心文章库、添加文章、提取结果三页的进一步布局与默认值修正；添加文章页默认配置筛除归档项并默认选中当前可用配置；“开始添加”区域扩为整行并将任务参数移到右侧；“任务管理”仅显示未结束任务及其控制；提取结果和文章库继续移除“页面用途”壳层；文章列表总数 badge 布局收紧避免折行。
- 关键设计决定: 配置选择以“当前可用配置”为事实源，不把已归档配置继续暴露给普通用户；任务管理区不再重复展示摘要指标，而是直接聚焦未结束任务列表和暂停/恢复/取消等操作；提取结果页与文章库进一步压缩无效说明区块，优先保留当前任务和审核主操作。
- 数据库迁移: 无。
- 兼容处理: 仅调整研究中心产品态展示与默认行为；旧 `/articles/*` 非产品态工作台与既有 API/Job 契约保持不变。
- 已运行测试: `cd web && CI=1 PATH=${NODE18_BIN}:$PATH pnpm test -- src/pages/research/index.test.tsx src/pages/articles/index.test.tsx`；`cd web && PATH=${NODE18_BIN}:$PATH pnpm typecheck`
- 测试结果: focused research/articles tests passed；frontend `typecheck` passed。
- 未完成项: 无。
- 已知风险: 添加文章页当前任务管理按所选任务类型查询未结束任务，仍不是“仅当前提交实例”的单独视图；如后续需要更严格的单次任务链路聚焦，需要在正式 Job 查询契约上单独扩展。
- 验收结论: `已完成`。本次跟进把研究中心三页进一步收口到更紧凑的业务操作界面，并修正了默认配置误落到归档项的问题。

- Task ID: `POST-DELIVERY-HOTFIX Research center page structure cleanup`
- 状态: `已完成`
- 修改范围: 研究中心三页产品态结构调整；文章库移除输入/处理状态卡并把输出改为“文章列表”；添加文章页改为“开始添加”与“任务管理”双区块；提取结果页移除输入/处理状态和外层输出壳层，主卡统一为“文章分析与审核”；研究中心与文章页相关前端测试、共享页面壳层可配置标题/显隐扩展。
- 关键设计决定: 不新建第二套研究中心实现；继续复用现有 `ArticlePipelinePage` / `ArticleResultsJourneyPage` 真实能力，只在产品态重排页面结构；“选择任务”复用原 `step` 选择机制并改为用户可理解的任务文案；添加文章提交成功后不再自动离开当前页，而是在本页任务管理区查看进度与控制。
- 数据库迁移: 无。
- 兼容处理: 非产品态 `/articles/*` 工作台保持既有行为；共享 `BusinessPageShell` / `ProductPageAdapter` 新增可选标题和显隐配置，默认行为保持原样，不影响未迁移页面。
- 已运行测试: `cd web && CI=1 PATH=${NODE18_BIN}:$PATH pnpm test -- src/pages/research/index.test.tsx src/pages/articles/index.test.tsx src/pages/product-entry-pages.test.tsx`；`cd web && PATH=${NODE18_BIN}:$PATH pnpm typecheck`
- 测试结果: research/articles/product-entry focused tests `29 passed`；frontend `typecheck` passed。
- 未完成项: 无。
- 已知风险: 添加文章页的“任务管理”当前聚合的是所选任务类型最近任务，并复用系统任务控制能力；如果后续需要更细的“仅本次提交链路”过滤或更丰富的任务摘要，需要在现有正式任务管理契约上单独扩展，而不是回退到内部技术页。
- 验收结论: `已完成`。研究中心三页已按当前正式信息架构收口为更直接的业务页面，避免普通用户先看到输入/处理状态通用壳层。

- Task ID: `DATA-CLEANUP-RAW-BLOG-ARTICLES-SIGNALS`
- 状态: `已完成`
- 修改范围: 本地 PostgreSQL `trade_strategy_ai` 中 `raw_articles`、`blog_articles`、`article_revisions`、`signals` 四张表的数据清理。
- 关键设计决定: 先核对外键依赖与依赖表残留，再按最小影响顺序删除父表与独立表数据；未使用 `TRUNCATE ... CASCADE`，避免误删未请求的下游表数据。
- 数据库迁移: 无。
- 兼容处理: 无 schema 变更；仅清空指定业务数据。
- 已运行测试: 删除前后依赖表核对；`raw_articles`、`blog_articles`、`article_revisions`、`signals` 行数复查。
- 测试结果: 删除事务成功提交；四张目标表删除前计数分别为 `131/131/131/4`，删除后均为 `0`。
- 未完成项: 无。
- 已知风险: 这四张表中的历史文章与信号记录已不可恢复，后续如需重建需要重新导入或回灌数据。
- 验收结论: `已完成`。目标表数据已清空且复查通过。

- Task ID: `POST-DELIVERY-HOTFIX Home business flow links`
- 状态: `已完成`
- 修改范围: 首页“从文章到盘后的业务流程”卡片改为正式业务入口链接；覆盖 `导入文章`、`提取规则`、`人工审核`、`历史回测`、`市场状态验证`、`作者画像`、`策略发布`、`今日盘前`、`今日盘后`；前端首页 focused tests。
- 关键设计决定: 保持首页流程文案不变；`策略发布` 先跳转到正式策略中心 `/strategies`，不新增独立“发布策略”页面；所有跳转仅指向 canonical 正式入口，不指向旧兼容页或内部技术页。
- 数据库迁移: 无。
- 兼容处理: 无 API 或路由契约变更；仅把首页静态流程说明升级为可点击导航，继续复用既有正式页面。
- 已运行测试: `cd web && CI=1 PATH=${NODE18_BIN}:$PATH pnpm test -- src/features/home/home-dashboard.test.tsx src/pages/home/index.test.tsx`
- 测试结果: focused homepage tests `4 passed`；测试输出包含 React Router v7 future flag warning，但不影响当前通过结果。
- 未完成项: 无。
- 已知风险: `提取规则`、`历史回测`、`市场状态验证`、`策略发布` 当前仍落到最贴近的正式聚合页，而不是独立步骤页；这是当前信息架构的真实状态，不应误解为已建立单独页面。
- 验收结论: `已完成`。首页流程区块现在可直接跳转到正式业务入口，`策略发布` 保持原文案并先进入策略中心。
...
- 已运行测试: `cd web && PATH=${NODE18_BIN}:$PATH pnpm test -- src/components/layout/business-page-shell.test.tsx src/components/layout/product-page-adapter.test.tsx src/pages/product-page-state-matrix.test.tsx src/pages/authors/index.test.tsx src/app/route-config.test.tsx src/pages/system/index.test.tsx src/pages/research/index.test.tsx`；`cd web && PATH=${NODE18_BIN}:$PATH pnpm typecheck`；`cd web && PATH=${NODE18_BIN}:$PATH pnpm lint`；`git diff --check`
- 测试结果: combined layout/adapter/state matrix/authors/route/system/research focused suite `78 passed`；frontend typecheck passed；frontend lint passed；`git diff --check` passed。Gate review 期间恢复 `/research/results` 当前 article-analysis 审核动作回归测试，避免 Task 1 shell 调整降低既有业务动作覆盖。
