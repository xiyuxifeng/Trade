...
- 新增 B：`TaskList` 存在明显状态漂移，Stage 0 的文档一致性要求被破坏
...
| 6 | 文档与验收状态收敛 | P1-1 / P1-3 | 修正 `TaskList`、`review` 和实际代码状态的漂移，避免后续 review 再出现“文档已完成、代码未闭环”。 |
| 7 | A 股实际约束与验证增强 | P2-1 | 在主闭环稳定后，再补更细粒度的 A 股约束和更真实的依赖验证。 |
