...
- [x] 数据库迁移、数据备份与恢复以 Job 方式创建并跳转 Job Detail。
- [x] 权限与审计、用户管理、系统健康检查不创建 Job，直接走页面内查询 / 提交。
- [x] 系统管理页面包含 `数据库迁移`、`数据备份与恢复`、`权限与审计`、`用户管理`、`系统健康检查` 五个功能。

### Task 4: 数据库迁移、备份、恢复与白名单

- [x] 数据库迁移、数据备份与恢复从工作流页迁移到系统管理页。
- [x] 数据备份与恢复都使用 `profile_id` 作为必填项。
...
- [x] 所有高风险操作都有确认弹窗。
- [x] 数据库迁移、数据备份与恢复提交后创建 Job，并可跳转 Job Detail。
