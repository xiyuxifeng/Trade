...
单机本机验证可以通过 `scripts/web_local.py` 完成构建、迁移和启动，Docker/Compose 仍然作为默认生产交付路径。

部署拓扑的长期基线说明见 [WebDeployment.md](WebDeployment.md)。Stage 9 后续任务都应以该文档为准，不再重复定义单机与内网部署边界。
...
11. 补齐生产部署、健康检查、备份恢复和回滚。`WEB-S9-001` 负责先定义部署拓扑基线。
12. 完成端到端验收与文档闭环。
...
14. `WEB-S9-001` 定义的部署拓扑与 `docs/WebDeployment.md` 保持一致，且作为后续 Stage 9 部署类任务的引用基线。
