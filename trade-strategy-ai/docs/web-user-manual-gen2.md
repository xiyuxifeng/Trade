...
> 说明：告警相关环境变量会在实际发送测试告警时再校验；缺失时会返回配置提示，不会让整个 Web 运行态直接 500。
...
> 说明：Kaipan 相关环境变量会在实际执行 `kaipan-fetch` / `kaipan-normalize` / `kaipan-run` 时再校验；缺失时会返回配置提示，而不是把 Web 运行态打成 500。
