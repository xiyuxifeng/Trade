...
- 告警相关环境变量会在实际发送测试告警时再校验；缺失时会返回配置提示，不会把整个 Web 启动流程打成 500。
...
- Kaipan 相关环境变量会在实际执行 `kaipan-fetch` / `kaipan-normalize` / `kaipan-run` 时再校验。
- 如果缺少 `KAIPAN_TOKEN`、`KAIPAN_USER_ID` 等配置，页面会返回明确的配置提示，而不是把整个 Web 运行态打成 500。
