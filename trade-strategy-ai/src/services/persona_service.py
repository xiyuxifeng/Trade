    ...
    async def build_sample_clusters(
        ...
        *,
        ...
        if profile_id is not None and str(profile_id).strip():
            ...
            config_file = str(loaded.config_path)

        trader_ids = [t.trader_id for t in loaded_config.traders]
        clusters: PersonaClustersFile = build_sample_clusters_file(trader_ids=trader_ids)

        path = dest
        if path is None:
            if loaded_config.persona.clusters_path:
                path = Path(loaded_config.persona.clusters_path)
            else:
        ...
        if from_akshare:
            try:
                ...
                return ServiceResult(
                    ...
                    payload={"profile_id": resolved_profile_id, "config_path": config_file, "base_dir": str(base_dir)},
                )
        else:
            cache_dir = base_dir / loaded_config.data.market_data_cache_dir
            ...
            if cached_csv.exists():
                try:
                    ...
                    return ServiceResult(
                        ...
                        payload={"profile_id": resolved_profile_id, "config_path": config_file, "base_dir": str(base_dir), "cache_path": str(cached_csv)},
                    )
            else:
                return ServiceResult(
                    status="error",
