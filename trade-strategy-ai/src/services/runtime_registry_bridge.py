...
def list_pipeline_contracts() -> list[dict[str, Any]]:
    ...
    return [_pipeline_contract(definition) for definition in (*ARTICLE_PIPELINE_SPECS, *STRATEGY_PIPELINE_SPECS)]


def get_pipeline_contract(pipeline_id: str) -> dict[str, Any] | None:
    """按 pipeline_id 获取 Pipeline canonical contract。"""
    for definition in (*ARTICLE_PIPELINE_SPECS, *STRATEGY_PIPELINE_SPECS):
