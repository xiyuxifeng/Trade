from __future__ import annotations

from contextlib import asynccontextmanager
from datetime import UTC, date, datetime
from pathlib import Path
from types import SimpleNamespace
from uuid import UUID, uuid4

import pytest
from sqlalchemy.ext.asyncio import AsyncSession, async_sessionmaker, create_async_engine
from sqlalchemy.ext.compiler import compiles
from sqlalchemy.dialects.postgresql import JSONB

from src.domain.enums import AuthorProfileKind, FormalLifecycleState, QualityStatus
from src.models.market_data_snapshot import MarketSnapshot
from src.models.market_data_snapshot_section import MarketSnapshotSection
from src.models.market_regime_record import MarketRegimeRecord, RegimeLabelRecord
from src.models.rule_applicability import RuleApplicabilityProfile, RuleApplicabilityRegimeRecord
from src.models.stage2_canonical import (
    AuthorProfileVersion,
    Authors,
    DailyRuleSelection,
    DailyRuleSelectionItem,
    DatasetLifecycleState,
    DatasetSnapshot,
    Rule,
    RuleVersion,
    Strategy,
    StrategyRuleMembership,
    StrategyVersion,
)


@compiles(JSONB, "sqlite")
def _compile_jsonb_sqlite(_type, compiler, **kw):
    return compiler.visit_JSON(None, **kw)


async def _build_session_factory(tmp_path: Path):
    engine = create_async_engine(f"sqlite+aiosqlite:///{tmp_path / 'daily_rule_selection.db'}")
    async with engine.begin() as conn:
        await conn.run_sync(Authors.__table__.create)
        await conn.run_sync(Rule.__table__.create)
        await conn.run_sync(RuleVersion.__table__.create)
        await conn.run_sync(DatasetSnapshot.__table__.create)
        await conn.run_sync(MarketSnapshot.__table__.create)
        await conn.run_sync(MarketSnapshotSection.__table__.create)
        await conn.run_sync(MarketRegimeRecord.__table__.create)
        await conn.run_sync(AuthorProfileVersion.__table__.create)
        await conn.run_sync(Strategy.__table__.create)
        await conn.run_sync(StrategyVersion.__table__.create)
        await conn.run_sync(StrategyRuleMembership.__table__.create)
        await conn.run_sync(RuleApplicabilityProfile.__table__.create)
        await conn.run_sync(DailyRuleSelection.__table__.create)
        await conn.run_sync(DailyRuleSelectionItem.__table__.create)
    session_factory = async_sessionmaker(engine, class_=AsyncSession, expire_on_commit=False)

    @asynccontextmanager
    async def _session_scope():
        async with session_factory() as session:
            try:
                yield session
                await session.commit()
            except Exception:
                await session.rollback()
                raise

    return _session_scope, session_factory, engine


def _readiness_view(
    *,
    trade_date: str,
    strategy_version_id: UUID,
    dataset_snapshot_id: UUID,
    market_snapshot_id: UUID,
    market_state_id: UUID,
    rule_applicability_profile_ids: list[UUID],
    author_method_profile_version_id: UUID,
    author_rule_profile_version_id: UUID,
    author_validated_profile_version_id: UUID,
    readiness_status: str = "ready",
    state: str = "ready",
    can_proceed: bool = True,
    can_proceed_in_degraded_mode: bool = False,
    data_quality_state: str = "ready",
    checks: list[dict] | None = None,
):
    checks = checks or []
    return SimpleNamespace(
        state=state,
        readiness_status=readiness_status,
        trade_date=trade_date,
        slot="09-25",
        summary_title="已就绪" if readiness_status == "ready" else "可降级继续",
        happened="正式盘前输入已检查。",
        affected="将用于生成每日规则选择。",
        repair_guidance="无需修复。",
        can_proceed=can_proceed,
        can_proceed_in_degraded_mode=can_proceed_in_degraded_mode,
        checks=[SimpleNamespace(**item) for item in checks],
        traceability=SimpleNamespace(
            trade_date=trade_date,
            strategy_version_id=str(strategy_version_id),
            dataset_snapshot_id=str(dataset_snapshot_id),
            market_snapshot_id=str(market_snapshot_id),
            market_state_id=str(market_state_id),
            rule_applicability_profile_ids=[str(item) for item in rule_applicability_profile_ids],
            author_method_profile_version_id=str(author_method_profile_version_id),
            author_rule_profile_version_id=str(author_rule_profile_version_id),
            author_validated_profile_version_id=str(author_validated_profile_version_id),
            data_quality_state=data_quality_state,
        ),
        repair_actions=[],
        warnings=[],
    )


class _FakeReadinessService:
    def __init__(self, readiness):
        self.readiness = readiness
        self.calls: list[dict[str, str]] = []

    async def get_readiness(self, trade_date: str, *, actor_id: str, actor_role: str):
        self.calls.append({"trade_date": trade_date, "actor_id": actor_id, "actor_role": actor_role})
        return self.readiness


async def _seed_selection_bundle(session_factory: async_sessionmaker[AsyncSession]) -> dict[str, UUID]:
    author_id = uuid4()
    method_profile_id = uuid4()
    rule_profile_id = uuid4()
    validated_profile_id = uuid4()
    dataset_snapshot_id = uuid4()
    market_snapshot_id = uuid4()
    market_state_id = uuid4()
    strategy_id = uuid4()
    strategy_version_id = uuid4()
    now = datetime(2026, 6, 21, 8, 30, tzinfo=UTC)

    rule_ids = [uuid4(), uuid4(), uuid4()]
    rule_version_ids = [uuid4(), uuid4(), uuid4()]
    membership_ids = [uuid4(), uuid4(), uuid4()]
    applicability_profile_ids = [uuid4(), uuid4()]

    async with session_factory() as session:
        session.add(Authors(author_id=author_id, source="seed", source_author_key="author-1", display_name="测试作者"))
        for idx, (rule_id, rule_version_id) in enumerate(zip(rule_ids, rule_version_ids, strict=True), start=1):
            session.add(
                Rule(
                    rule_id=rule_id,
                    business_key=f"rule-{idx}",
                    current_published_version_id=rule_version_id,
                    created_at=now,
                    created_by="seed",
                    updated_at=now,
                    updated_by="seed",
                )
            )
            session.add(
                RuleVersion(
                    rule_version_id=rule_version_id,
                    rule_id=rule_id,
                    version_no=1,
                    canonical_fingerprint=f"rule-fingerprint-{idx}",
                    schema_version="rule-schema-v1",
                    lifecycle_state=FormalLifecycleState.published,
                    title=f"规则{idx}",
                    description=f"规则{idx}说明",
                    rule_type="entry",
                    instrument_scope={"market": "CN"},
                    condition_json={"indicator": f"indicator-{idx}"},
                    action_json={"decision": "buy"},
                    parameter_json={"lookback_days": 20},
                    data_dependencies={"datasets": ["ohlcv_1d"]},
                    evidence_json={"source": "formal"},
                    quality_status=QualityStatus.verified,
                    published_at=now,
                    published_by="seed",
                    created_by="seed",
                    updated_by="seed",
                )
            )

        session.add(
            DatasetSnapshot(
                dataset_snapshot_id=dataset_snapshot_id,
                content_fingerprint="dataset-fingerprint-1",
                trade_date=date(2026, 6, 20),
                market="CN",
                dataset_type="ohlcv_1d",
                date_from=date(2026, 1, 1),
                date_to=date(2026, 6, 20),
                symbol_manifest={"count": 100},
                ohlcv_manifest={"coverage": "complete"},
                kaipan_manifest={"pre_market_trade_date": "2026-06-21"},
                benchmark_symbol="000300.SH",
                market_state_definition_version="market-state-v2",
                available_at=now,
                frozen_at=now,
                lifecycle_state=DatasetLifecycleState.ready,
                storage_ref={"logical_dataset_id": "dataset-ready"},
            )
        )
        session.add(
            MarketSnapshot(
                id=market_snapshot_id,
                snapshot_id="market-snapshot-2026-06-21-am",
                trade_date=date(2026, 6, 21),
                market="CN",
                profile_id="profile-default",
                data_version="v2",
                slot="09-25",
                quality_status="ready",
                provider_sources=["kaipan"],
                section_count=6,
                available_section_count=6,
                partial_section_count=0,
                missing_section_count=0,
                storage_ref={"snapshot_id": "market-snapshot-2026-06-21-am"},
                data_quality={"state": "verified"},
                captured_at=now,
                available_at=now,
                effective_at=now,
                content_fingerprint="market-fingerprint-1",
                manifest_json={"slot": "09-25"},
            )
        )
        session.add(
            MarketRegimeRecord(
                market_state_id=market_state_id,
                regime_id="market-state-ready",
                market_snapshot_id=market_snapshot_id,
                snapshot_id="market-snapshot-2026-06-21-am",
                trade_date=date(2026, 6, 21),
                market="CN",
                regime_version="market-state-v2",
                source_feature_version="features-v2",
                primary_label="强势上行",
                labels=[
                    RegimeLabelRecord(
                        label="强势上行",
                        label_type="primary",
                        score=0.9,
                        confidence=0.9,
                        status="ready",
                        reason="趋势向上",
                    )
                ],
                features=[],
                confidence=0.82,
                quality_status="ready",
                storage_ref={"market_state_id": "market-state-ready"},
                available_at=now,
                created_at=now,
                updated_at=now,
            )
        )
        for profile_id, profile_kind in (
            (method_profile_id, AuthorProfileKind.method),
            (rule_profile_id, AuthorProfileKind.rule),
            (validated_profile_id, AuthorProfileKind.validated),
        ):
            session.add(
                AuthorProfileVersion(
                    author_profile_version_id=profile_id,
                    author_profile_id=uuid4(),
                    author_id=author_id,
                    profile_kind=profile_kind,
                    version_no=1,
                    schema_version="author-profile-v1",
                    lifecycle_state=FormalLifecycleState.published,
                    payload={"summary": f"{profile_kind.value}-summary"},
                    evidence_json={"source": "formal"},
                    source_article_ids={},
                    source_rule_version_ids={},
                    source_rule_family_ids={},
                    source_applicability_profile_ids={},
                    source_backtest_run_ids={},
                    source_backtest_result_ids={},
                    source_daily_review_ids={},
                    source_versions_json={"schema_version": "author-profile-v1"},
                    quality_status=QualityStatus.verified,
                    review_status="published",
                    published_at=now,
                    published_by="seed",
                    created_by="seed",
                    updated_by="seed",
                )
            )
        session.add(
            Strategy(
                strategy_id=strategy_id,
                owner_type="platform",
                owner_id=None,
                business_key="cn-swing-core",
                current_published_version_id=strategy_version_id,
                created_at=now,
                created_by="seed",
                updated_at=now,
                updated_by="seed",
            )
        )
        session.add(
            StrategyVersion(
                strategy_version_id=strategy_version_id,
                strategy_id=strategy_id,
                version_no=3,
                schema_version="strategy-schema-v1",
                lifecycle_state=FormalLifecycleState.published,
                title="A股趋势轮动策略",
                summary="正式策略",
                risk_policy_json={"position_limit": 0.2},
                selection_policy_json={"degraded_mode": "allow_partial_rule_applicability"},
                universe_json={"market": "CN"},
                author_method_profile_version_id=method_profile_id,
                author_rule_profile_version_id=rule_profile_id,
                author_validated_profile_version_id=validated_profile_id,
                evidence_json={"validation_summary": {"state": "passed"}},
                quality_status=QualityStatus.verified,
                review_status="published",
                published_at=now,
                published_by="seed",
                created_by="seed",
                updated_by="seed",
            )
        )
        for idx, (membership_id, rule_version_id, weight) in enumerate(
            zip(membership_ids, rule_version_ids, (0.9, 0.6, 0.2), strict=True),
            start=1,
        ):
            session.add(
                StrategyRuleMembership(
                    membership_id=membership_id,
                    strategy_version_id=strategy_version_id,
                    rule_version_id=rule_version_id,
                    base_weight=weight,
                    status="active",
                    configuration_json={"priority": idx},
                )
            )

        session.add(
            RuleApplicabilityProfile(
                applicability_profile_id=applicability_profile_ids[0],
                rule_version_id=rule_version_ids[0],
                rule_version_fingerprint="rule-fingerprint-1",
                rule_version_no=1,
                dataset_snapshot_id=dataset_snapshot_id,
                dataset_fingerprint="dataset-fingerprint-1",
                lifecycle_state=FormalLifecycleState.published,
                result_status="ready",
                rule_id="rule-1",
                profile_version="rule-applicability-v1",
                source_backtest_id="backtest-run-1",
                source_rule_version="v1",
                market_regime_version="market-state-v2",
                source_feature_version="features-v2",
                source_backtest_run_ids=["backtest-run-1"],
                source_backtest_result_ids=["backtest-result-1"],
                source_result_fingerprints=["backtest-fingerprint-1"],
                market_snapshot_ids=[str(market_snapshot_id)],
                market_snapshot_fingerprints=["market-fingerprint-1"],
                sample_count=50,
                eligible_sample_count=50,
                evaluated_sample_count=50,
                coverage=1.0,
                return_metric=0.12,
                win_rate=0.58,
                maximum_drawdown=0.08,
                recommendation_status="available",
                quality_status="verified",
                insufficient_sample_status="sufficient",
                review_status="published",
                reviewed_by="seed",
                reviewed_at=now,
                created_by="seed",
                applicable_regimes=[
                    RuleApplicabilityRegimeRecord(
                        regime_label="强势上行",
                        decision="selected",
                        score=0.9,
                        sample_count=50,
                        win_rate=0.58,
                        avg_return=0.12,
                        avg_win_return=None,
                        avg_loss_return=None,
                        max_drawdown=0.08,
                        profit_factor=None,
                        confidence=0.9,
                        low_sample=False,
                        reason="当前市场状态匹配。",
                        evidence=["market-state-ready"],
                    )
                ],
            )
        )
        session.add(
            RuleApplicabilityProfile(
                applicability_profile_id=applicability_profile_ids[1],
                rule_version_id=rule_version_ids[1],
                rule_version_fingerprint="rule-fingerprint-2",
                rule_version_no=1,
                dataset_snapshot_id=dataset_snapshot_id,
                dataset_fingerprint="dataset-fingerprint-1",
                lifecycle_state=FormalLifecycleState.published,
                result_status="insufficient_sample",
                rule_id="rule-2",
                profile_version="rule-applicability-v1",
                source_backtest_id="backtest-run-2",
                source_rule_version="v1",
                market_regime_version="market-state-v2",
                source_feature_version="features-v2",
                source_backtest_run_ids=["backtest-run-2"],
                source_backtest_result_ids=["backtest-result-2"],
                source_result_fingerprints=["backtest-fingerprint-2"],
                market_snapshot_ids=[str(market_snapshot_id)],
                market_snapshot_fingerprints=["market-fingerprint-1"],
                sample_count=4,
                eligible_sample_count=4,
                evaluated_sample_count=4,
                coverage=0.4,
                return_metric=0.02,
                win_rate=0.5,
                maximum_drawdown=0.05,
                recommendation_status="available",
                quality_status="partial",
                insufficient_sample_status="insufficient_sample",
                review_status="published",
                reviewed_by="seed",
                reviewed_at=now,
                created_by="seed",
                neutral_regimes=[
                    RuleApplicabilityRegimeRecord(
                        regime_label="强势上行",
                        decision="reduced",
                        score=0.4,
                        sample_count=4,
                        win_rate=0.5,
                        avg_return=0.02,
                        avg_win_return=None,
                        avg_loss_return=None,
                        max_drawdown=0.05,
                        profit_factor=None,
                        confidence=0.4,
                        low_sample=True,
                        reason="样本不足。",
                        evidence=["backtest-run-2"],
                    )
                ],
            )
        )
        await session.commit()

    return {
        "strategy_version_id": strategy_version_id,
        "dataset_snapshot_id": dataset_snapshot_id,
        "market_snapshot_id": market_snapshot_id,
        "market_state_id": market_state_id,
        "method_profile_id": method_profile_id,
        "rule_profile_id": rule_profile_id,
        "validated_profile_id": validated_profile_id,
        "rule_version_ids": rule_version_ids,
        "membership_ids": membership_ids,
        "applicability_profile_ids": applicability_profile_ids,
    }


@pytest.mark.asyncio()
async def test_daily_rule_selection_is_deterministic_and_never_selects_missing_applicability_rule(tmp_path: Path) -> None:
    from src.services.daily_rule_selection_service import DailyRuleSelectionService

    session_scope, session_factory, engine = await _build_session_factory(tmp_path)
    try:
        seeded = await _seed_selection_bundle(session_factory)
        readiness = _readiness_view(
            trade_date="2026-06-21",
            strategy_version_id=seeded["strategy_version_id"],
            dataset_snapshot_id=seeded["dataset_snapshot_id"],
            market_snapshot_id=seeded["market_snapshot_id"],
            market_state_id=seeded["market_state_id"],
            rule_applicability_profile_ids=seeded["applicability_profile_ids"],
            author_method_profile_version_id=seeded["method_profile_id"],
            author_rule_profile_version_id=seeded["rule_profile_id"],
            author_validated_profile_version_id=seeded["validated_profile_id"],
        )
        service = DailyRuleSelectionService(
            session_scope_factory=session_scope,
            readiness_service=_FakeReadinessService(readiness),
        )

        first = await service.get_rule_selection("2026-06-21", actor_id="tester", actor_role="viewer")
        second = await service.get_rule_selection("2026-06-21", actor_id="tester", actor_role="viewer")

        assert first.selection_status == "degraded"
        assert first.daily_rule_selection_id == second.daily_rule_selection_id
        assert [item.rule_version_id for item in first.enabled_rules] == [str(seeded["rule_version_ids"][0])]
        assert [item.rule_version_id for item in first.reduced_rules] == [str(seeded["rule_version_ids"][1])]
        assert [item.rule_version_id for item in first.suspended_rules] == [str(seeded["rule_version_ids"][2])]
        assert first.reduced_rules[0].controlling_priority_tier == "formal_rule_applicability"
        assert first.suspended_rules[0].controlling_priority_tier == "formal_rule_applicability"
        assert first.suspended_rules[0].unresolved_inputs == ["missing_rule_applicability"]
        assert first.enabled_rules[0].reason_tiers == second.enabled_rules[0].reason_tiers
        assert first.reduced_rules[0].reason_tiers == second.reduced_rules[0].reason_tiers
        assert first.suspended_rules[0].reason_tiers == second.suspended_rules[0].reason_tiers
        assert first.traceability.rule_applicability_profile_ids == [
            str(seeded["applicability_profile_ids"][0]),
            str(seeded["applicability_profile_ids"][1]),
        ]
    finally:
        await engine.dispose()


@pytest.mark.asyncio()
async def test_daily_rule_selection_reports_blocked_readiness_without_persisting_selection(tmp_path: Path) -> None:
    from src.services.daily_rule_selection_service import DailyRuleSelectionService

    session_scope, session_factory, engine = await _build_session_factory(tmp_path)
    try:
        seeded = await _seed_selection_bundle(session_factory)
        readiness = _readiness_view(
            trade_date="2026-06-21",
            strategy_version_id=seeded["strategy_version_id"],
            dataset_snapshot_id=seeded["dataset_snapshot_id"],
            market_snapshot_id=seeded["market_snapshot_id"],
            market_state_id=seeded["market_state_id"],
            rule_applicability_profile_ids=seeded["applicability_profile_ids"],
            author_method_profile_version_id=seeded["method_profile_id"],
            author_rule_profile_version_id=seeded["rule_profile_id"],
            author_validated_profile_version_id=seeded["validated_profile_id"],
            readiness_status="blocked",
            state="unavailable",
            can_proceed=False,
            data_quality_state="blocked",
            checks=[
                {
                    "code": "kaipan_pre_market",
                    "label": "Kaipan 盘前数据",
                    "status": "blocked",
                    "happened": "今日盘前市场快照缺失。",
                    "affected": "系统无法确认盘前市场输入。",
                    "repair_guidance": "先补齐盘前市场数据。",
                    "can_proceed_in_degraded_mode": False,
                    "traceability": {"market_snapshot_id": None},
                }
            ],
        )
        service = DailyRuleSelectionService(
            session_scope_factory=session_scope,
            readiness_service=_FakeReadinessService(readiness),
        )

        result = await service.get_rule_selection("2026-06-21", actor_id="tester", actor_role="viewer")

        assert result.selection_status == "blocked"
        assert result.state == "unavailable"
        assert result.generated is False
        assert result.daily_rule_selection_id is None
        assert result.happened == "正式盘前检查尚未通过，当前不能生成每日规则选择。"
        async with session_factory() as session:
            persisted = (await session.execute(DailyRuleSelection.__table__.select())).all()
        assert persisted == []
    finally:
        await engine.dispose()
