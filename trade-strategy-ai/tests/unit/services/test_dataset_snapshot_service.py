...
@pytest.mark.asyncio()
async def test_freeze_ohlcv_snapshot_creates_new_version_when_row_payload_changes(
    dataset_snapshot_service_session_factory,
) -> None:
    """Changing OHLCV content for the same symbol/date must create a new snapshot version."""
    ...
    async with dataset_snapshot_service_session_factory() as session:
        row = await session.scalar(select(OHLCVBar).where(OHLCVBar.symbol == "000001.SZ"))
        assert row is not None
        row.close = 10.8
        row.source_payload_fingerprint = "row-1-repaired"
        row.ingested_at = datetime(2026, 4, 1, 8, 20, tzinfo=UTC)
        await session.commit()
    ...
    assert first.content_fingerprint != second.content_fingerprint
    assert first.dataset_snapshot_id != second.dataset_snapshot_id
    assert second.ohlcv_manifest["row_fingerprints"] == ["row-1-repaired"]
