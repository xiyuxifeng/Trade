...
export function MarketStateCenter() {
  ...
  const benchmarkOptionsQuery = useQuery({
    queryKey: ['market-state', 'benchmark-options'],
  ...
  return (
    <section className="dashboard-grid">
      <Card className="xl:col-span-5">
          ...
          <CardDescription>构建 MarketState 快照，先选择 benchmark，再决定是否从 AkShare 拉取或使用缓存。</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <label className="space-y-2 text-sm text-slate-300">
            ...
            <p className="text-xs text-slate-500">该基准会写入 MarketState 结果，作为后续回测和复现的事实源。</p>
          </label>
          <label className="space-y-2 text-sm text-slate-300">
            <span>As of</span>
